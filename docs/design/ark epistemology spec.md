...
- **Scope:** Classifier logic · Contradiction protocol · Darśana plurality rule · Site information architecture · Three worked examples
- **Status:** Draft for engineering handoff. Ontological claims herein are stated positions of the ĀRK project, not neutral academic claims — see §1.4.
...
Flagged throughout this document, collected here for a single review pass:

1. **§1.3** — confirm the five-tattva enum is complete and final for the Topic layer.
2. **§3.3** — decide whether the "asymptote" claim (science converging toward śabda over time) should be built as an active engine feature (e.g., a trending "distance from resolution" score) or remain outside the mechanical classifier as editorial commentary only.
3. **§5.2** — confirm the proposed intra-Vedānta plurality extension (showing rival bhāṣya readings at Tier 2, with Govinda-bhāṣya specifically — not "Vedānta" unqualified — as the Tier 3 root node).
4. **General** — this spec assumes a human or AI reviewer applies the four-layer test at ingestion time; it does not yet specify confidence thresholds for when an AI-automated tagging pipeline should auto-publish versus route to the §2.5 review queue. Worth scoping before the ingestion pipeline is built.
