...
export function BriefRow({ brief, free = false }: { brief: Brief; free?: boolean }) {
  ...
  const inner = (
    <div className="flex flex-col gap-16 py-32 md:flex-row md:items-baseline md:justify-between md:gap-32">
      <div className="min-w-0">
        ...
        <h3 className="mt-8 font-serif text-[26px] leading-tight text-bone">{brief.title}</h3>
        <p className="mt-8 max-w-measure font-serif text-reader text-ash">{brief.dek}</p>
      </div>
      {available && (
        <div className="flex shrink-0 items-center gap-24 md:flex-col md:items-end md:gap-8">
          ...
          <span className="font-sans text-[15px] text-bone">
  ...
  return (
    <Link
      ...
      className="block rule-h transition-colors duration-150 hover:bg-raise"
    >
      {inner}
    </Link>
  );
